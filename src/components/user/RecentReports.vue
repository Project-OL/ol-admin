...
function navigateToReports(kind?: 'nudity' | 'user_report') {
  const query: Record<string, string> = { userId: props.userId }
  if (kind) query.kind = kind
  else query.kind = 'user_report'
  router.push({ path: '/admin/live-moderation', query })
}
</script>
  ...
  <div class="admin-card">
    ...
    <div class="space-y-2">
      <button
        ...
        @click="navigateToReports(cat.key === 'nudity' ? 'nudity' : 'user_report')"
      >
